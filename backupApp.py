...
@app.route("/getAllListsInformation")
def getEveryListInformation():
...
def createNewList(list_name):
    return {"list_id": mainProgram().createNewList(list_name)}

@app.route("/createNewMovie/<list_id>", methods=["POST"])
def createNewMovie(list_id:int):
    ...
    try:
        newMovie_id = mainProgram().createNewMovie(title=title, year=year, kind=kind, cover_url=cover_url, imdb_id=imdb_id, rating=rating)
        mainProgram().putMovieOnList(newMovie_id, list_id)
